...
export default function PieChartForSummary({expenses} : IPieChartForSummaryProps) {
    const desktopOS = [
        {
            label: 'Windows',
            value: 72.72,
    ...
    const [summaryData, setSummaryData] = React.useState<{label: string, value: number}[]>(desktopOS);
    React.useEffect(() => {
...
function getSummaryData(expenses: OneExpenseSummaryTypeDeclaration[]): {label: string, value: number}[] {
        ...
        const nonEssentialFoodSpentPercentage = (nonEssentialFoodSpent / totalSpent) * 100;
        ...
        const recreationalPurchaseSpentPercentage = (recreationalPurchaseSpent / totalSpent) * 100;

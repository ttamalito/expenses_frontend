...
import {
  fetchExpensesOfATypeForAMonthPath,
  fetchAllExpensesForAMonthPath,
} from '../requests/paths';
...
export default function MonthExpenses() {
  ...
  // @ts-ignore
  const [budget, setBudget] = useState<monthBudgetType>({
    monthBudget: 0,
    ...
    <>
      <Box sx={{ flexGrow: 1 }}>
        <Grid container spacing={2}>
          <Grid size={{ xs: 12, md: 6, sm: 6 }}>{seeExpensesOfAType}</Grid>
      ...
      {!singleTypeFlag
        ...
        : 'Total Spent on month for a single type:'}
      {!singleTypeFlag ? totalSpent[0] : totalSpentOfASingleType}
      <br />
      {!singleTypeFlag
        ? `Your budget is: ${budget.monthBudget}`
        : `Your budget for this type is: ${budget.typesBudget[singleType as keyof typesBudgetTypeDeclaration]} euros`}
      ;
      <br />
...
function getAllExpenses(
  ...
  setExpenses: {
    (value: React.SetStateAction<OneExpenseSummaryTypeDeclaration[]>): void;
    (arg0: any[]): void;
  },
) {
 ...
 * @param {Function} setTotalEarned - A function to set the total earned.
 */
function fetchAllExpensesForAMonth(
  month: string | undefined,
  year: string | undefined,
  setExpenses: {
    (value: React.SetStateAction<OneExpenseSummaryTypeDeclaration[]>): void;
...
function getBudget(
  year: string | undefined,
  setBudget: {
    (value: React.SetStateAction<monthBudgetType>): void;
    (arg0: any): void;
  },
) {
...
function getExpensesOfAType(
  ...
  const url = fetchExpensesOfATypeForAMonthPath(
    year as string,
    month as string,
  );
  fetch(url, {
          ...
          <ul>
            {data.expenses.map(
              (expense: {
                ...
                const div = (
                  <div>
                    {expense.amount} {expense.type} {expense.notes}{' '}
                    {expense.date} <br />{' '}
                  </div>
                );

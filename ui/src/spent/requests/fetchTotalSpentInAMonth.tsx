 ...
 * @param type
 */
export default async function fetchTotalSpentInAMonth(
  year: number,
  month: number,
  type: string,
): Promise<IResponseWrapper<number>> {
  ...
  if (response.status === 404) {
    alert = createWarningAlert(
      'There was an error querying the total spent in a month',
    );
  }

  if (response.status === 400) {
    alert = createWarningAlert(
      'You provided wrong query parameters ' + data.message,
    );
  }

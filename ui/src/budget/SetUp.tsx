...
export default function SetUp() {
  ...
  const [errorWrapper, setErrorWrapper] =
    useState<IErrorWrapper>(defaultErrorWrapper);
  const [showAlert, setShowAlert] = useState<IShowAlertWrapper>(
    defaultShowAlertWrapper,
  );

  const successfullUpdateAlert = (
    <Alert severity="success" icon={<CheckIcon fontSize={'inherit'} />}>
      The budget was updated successfully{' '}
    </Alert>
  );
  ...
  const showAlertButton = (
    <button
      onClick={() => {
        return setShowAlert({ alert: successfullUpdateAlert, show: true });
      }}
    >
      Show Alert
    </button>
  ...
  return (
    <>
      ...
      {
        <SetUpForm
          typesBudget={budget.typesBudget}
...
function retrievePercentages(typesBudget: expensesTypesTypesDeclarations) {
  const total: number = Number(
    Object.values(typesBudget).reduce((accumulator, currentValue) => {

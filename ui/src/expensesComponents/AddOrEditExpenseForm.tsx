...
export default function AddOrEditExpenseForm({expense, submitData, edit}: IAddOrEditExpenseFormProps) {
    ...
    const amountInputField = <input type="text"
                                    placeholder={'amount'}
                                    name={'amount'}
                                    autoComplete={'off'}
                                    defaultValue={expense?.amount}
    />;
    const monthInputField = <input type="number"
                                   placeholder={'month'}
                                   name={'month'}/>;
    const yearInputField = <input type="number"
                                  placeholder={'year'}
                                  name={'year'}/>;
    const dateInputField = <FormControl required={true}>
    ...
    const notesInputField = <input type="text"
                                   placeholder={'notes'}
                                   name={'notes'}
                                   autoComplete={'off'}
    ...
    const button = <Button type={'submit'} variant={'contained'}
                           color="success">{!edit ? "Add an expense" : "Edit the expense"}</Button>;

    const components = <div>
        {amountInputField}
        {!edit && <><br/>{monthInputField}<br/>{yearInputField}</>}
        <br/>
        {dateInputField}
        <br/>
        {!edit && <>{transactionInputField}<br/></>}
        {typeInputField}
        <br/>
        {notesInputField}
        <br/>
